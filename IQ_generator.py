...
class Question ():
    ...
    def save_question_explication(self, explication, path):
        """ Save explication in a json file to a given path
            @params
            @self: self
            @explication: list
                         for each response, save its status and the explanation associated with its status
            @path: String
                        path to save the file
        """
        file_response = open(path,"w")
        json.dump(explication,file_response)
        file_response.close()
    
    def generate_possible_shifts(self, number_of_positions, distance_of_position):
        """ 
        creates an array of size number_of_position
        @params
        @self: self
    ...
    def generate_random_lines(self):
        """ Generates a sequence of horizontal and vertical lines with a random pattern
            Create seven wrong answers, the right answer and an explanation for each answer
            Save all these in a folder named "question"
            @params
            self: self
        """
...
sont en continuité par colonne et par ligne"""
 ...
 "Le trait horizontal doit être à la même position que le trait horizontal de chaque image sur la même ligne")
    ...
    def generate_random_boxes(self):
        """ Generates a sequence of box with a random pattern (evolution vertical and horizontal)
            Create seven wrong answers, the right answer and an explanation for each answer
            Save all these in a folder named "question"
            @params
            self: self
        ...
        true_response_cords = {"number_of_objects" : 0, "position_of_objects" : 0}
        for _ in range(3):
            ...
            for _ in range(3):
                dec = frame_dec
                true_response_cords["number_of_objects"] = counter
                for _ in range(counter):
                    ...
                    dec += int(size/3) 
                    true_response_cords["position_of_objects"] = possible_dec[next_pos_box]            
                if initial_number == 3 :
                    counter-=1
                else :
                    counter+=1
        ...
        while number_of_false_response < 7:
            ...
            if pos_box != true_response_cords["position_of_objects"] or columns_number != true_response_cords["number_of_objects"] :
    ...
    def generate_random_hexa(self):
        """ Generates a sequence of hexagon with a random pattern (vertical evolution)
            Create seven wrong answers, the right answer and an explanation for each answer
            Save all these in a folder named "question"
            @params
            self: self
